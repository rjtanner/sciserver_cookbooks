...
# ABC Guide for XMM-Newton -- Timing Mode Data Processing
<hr style="border: 2px solid #fadbac" />

- **Description:** XMM-Newton ABC Guide, Timing Mode Data Processing.
- **Level:** Beginner
...
<div class="alert alert-block alert-info">
    <b>Note:</b> Running <tt>epproc</tt> on this particular Obs ID will take A LONG TIME, depending on your machine. Be prepared to wait.
</div>
...
my_obs.basic_setup(overwrite=True,repo='sciserver',
                   run_rgsproc=False,run_emproc=False)
```
...
We also note that for this particular observation not all instruments were used. Which instruments were active for this observation is stored in a dictionary in the `ObsID` object. This data is contained in the ODF summary file, `sas_odf`.

```python
my_obs.active_instruments
```
...
We need to change into the work directory to run the next SAS tasks. We also get the name and path to the event list file created in §1.

```python
os.chdir(my_obs.work_dir)
pn_burst           = my_obs.files['PNevt_list'][0]
...
## 5. Create and Display a Light Curve
...
```python
my_obs.quick_lcplot(basic_filter_file, light_curve_file=basic_filter_ltcrv, timebinsize = 50)
```
...
## 6. Extract the Source and Background Spectra
...
```python
inargs = {'set'               : pn_spectra_file,
          'plotfile'          : 'pn_epat.ps',
          'useplotfile'       : 'yes',
          'withbackgroundset' : 'yes',
          'backgroundset'     : pn_bkg_file}
...
## 8. My Observation is Piled Up! Now What?
...
## 9. Determine the Spectrum Extraction Areas
...
## 10. Create the Photon Redistribution Matrix (RMF) and Ancillary File (ARF)
