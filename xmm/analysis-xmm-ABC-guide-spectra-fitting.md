...
## 1. Introduction
This tutorial is based on Chapter 12 from [The XMM-Newton ABC Guide](https://heasarc.gsfc.nasa.gov/docs/xmm/abc/ "ABC Guide") prepared by the NASA/GSFC XMM-Newton Guest Observer Facility. This notebook assumes you are at least minimally familiar with pySAS on SciServer (see the [Long pySAS Introduction](./analysis-xmm-long-intro.md "Long pySAS Intro")) and that you have already worked through the two Jupyter Notebooks on filtering EPIC data and extracting a spectra from a region ([link to first notebook](./analysis-xmm-ABC-guide-EPIC-image-filtering.md) and [link to second notebook](./analysis-xmm-ABC-guide-EPIC-source-spectrum.md)). In this tutorial we will demonstrate how to use PyXSPEC to load a spectra a set up a simple power law model.
